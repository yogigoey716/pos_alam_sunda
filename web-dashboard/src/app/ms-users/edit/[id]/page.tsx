...
function EditMsUsersPage() {
    ...
    useEffect(() => {
        ...
        msUsersService
            ...
            if (user) {
                ...
                setBranchesId(user.branch.id || "");
                // password biasanya tidak ditampilkan, tapi jika memang dikembalikan API bisa di-set
                // setPassword(user. || "");
            }
            })
            .catch((err) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await msUsersService.update(id, {
        username,
        email,
        role_id,
        branches_id,
...
export default withAuth(EditMsUsersPage);